...
public class PowerModuleAudio : MonoBehaviour {
	
	private FMOD.Studio.EventInstance powerModuleAudio;
	...
	void Start () {
		...
		powerModuleAudio = FMOD_StudioSystem.instance.GetEvent("event:/PowerModuleAmbience");
		powerModuleAudio.getParameter("AirlockPressure", out airlockPressure);
		powerModuleAudio.getParameter("Distance", out distance);
		audioStarted = false;
	...
	void Update () {
		...
		else
		{
			...
			{
				powerModuleAudio.stop(FMOD.Studio.STOP_MODE.IMMEDIATE);
				audioStarted = false;
			}
		}
